...
int[] randomIron = {(int)random(1, 5), (int)random(1, 5), (int)random(1, 4), (int)random(1, 3)};
...
int current_position = 0;//current position
int current =0;
...
void settings() {
  ...
  block_name[8] = "air";
  block_color[8] = color(131,206,255);
  block_name[9] = "dirt";
...
void draw() {
  ...
  if (keyPressed) {
    ...
    //open and saving binds
    else if (key == 'o' || key == 'O' || key == 'щ' || key == 'Щ') {
      try {
        openLevel("level");
      }
      catch(IOException ex) {
        System.err.println("Failed the open you world");
      }
